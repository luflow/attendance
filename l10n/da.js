/**
 * This file is auto-generated from translationfiles/da/attendance.po
 * Do not edit directly - edit the .po file instead and run: npm run l10n:convert
 */
OC.L10N.register(
    "attendance",
    {
        "Attendance": "Deltagelse",
        "Appointments": "Aftaler",
        "Upcoming Appointments": "Kommende aftaler",
        "Unanswered": "Ubesvaret",
        "Appointment Management": "Aftalehåndtering",
        "Loading appointments…": "Indlæser aftaler…",
        "By Group": "Efter gruppe",
        "Others": "Andre",
        "Show Past Appointments": "Vis tidligere aftaler",
        "Hide Past Appointments": "Skjul tidligere aftaler",
        "Create Appointment": "Opret aftale",
        "Edit Appointment": "Rediger aftale",
        "Edit": "Rediger",
        "Delete": "Slet",
        "Appointment Name": "Aftalenavn",
        "Description": "Beskrivelse",
        "You can use **bold** and *italic* formatting": "Du kan bruge **fed** og *kursiv* formatering",
        "Start": "Start",
        "End": "Slut",
        "Start Date & Time": "Startdato og -tid",
        "End Date & Time": "Slutdato og -tid",
        "Save": "Gem",
        "Cancel": "Annuller",
        "Yes": "Ja",
        "No": "Nej",
        "Maybe": "Måske",
        "Your Response": "Dit svar",
        "Response Summary": "Svaroversigt",
        "Permissions": "Tilladelser",
        "Configure which groups can perform specific actions. Users must belong to at least one of the selected groups to access the feature. If no group is selected, all users have access to the feature.": "Konfigurer hvilke grupper der kan udføre specifikke handlinger. Brugere skal tilhøre mindst én af de valgte grupper for at få adgang til funktionen. Hvis ingen gruppe er valgt, har alle brugere adgang til funktionen.",
        "Manage Appointments": "Administrer aftaler",
        "Groups that can create, update, and delete appointments": "Grupper der kan oprette, opdatere og slette aftaler",
        "Check-in Access": "Check-in adgang",
        "Groups that can access the check-in interface and execute check-ins": "Grupper der kan tilgå check-in grænsefladen og udføre check-ins",
        "See Response Overview": "Se svaroversigt",
        "Groups that can see the response overview with details": "Grupper der kan se svaroversigten med detaljer",
        "See Comments": "Se kommentarer",
        "Groups that can see comments in the response overview": "Grupper der kan se kommentarer i svaroversigten",
        "Check-in Permissions": "Check-in tilladelser",
        "Groups that can see comments": "Grupper der kan se kommentarer",
        "Users in these groups can see comments from other users": "Brugere i disse grupper kan se kommentarer fra andre brugere",
        "Appointment Reminders": "Aftalepåmindelser",
        "Send automatic reminder notifications to users who haven't responded to appointments.": "Send automatiske påmindelser til brugere, der ikke har svaret på aftaler.",
        "Enable automatic reminders": "Aktiver automatiske påmindelser",
        "Send reminders X days before appointment:": "Send påmindelser X dage før aftalen:",
        "(1-30 days)": "(1-30 dage)",
        "Configure attendance management and reminders": "Konfigurer deltagelseshåndtering og påmindelser",
        "Loading settings…": "Indlæser indstillinger…",
        "Days before appointment": "Dage før aftalen",
        "Send reminders this many days before the appointment (1-30 days)": "Send påmindelser dette antal dage før aftalen (1-30 dage)",
        "Reminder frequency (days)": "Påmindelsesfrekvens (dage)",
        "How often to remind users who haven't responded. Set to 0 to only remind once, or 1-30 to repeat reminders every N days.": "Hvor ofte brugere skal påmindes, hvis de ikke har svaret. Sæt til 0 for kun at minde én gang, eller 1-30 for at gentage påmindelser hver N. dag.",
        "Whitelisted Groups": "Gruppebegrænsning",
        "Select user groups that should be included in attendance statistics and check-in list filters. If no groups are selected, all groups are included.": "Vælg brugergrupper der skal inkluderes i deltagelsesstatistikker og check-in listefiltre. Hvis ingen grupper er valgt, inkluderes alle grupper.",
        "The Notifications app is not enabled. Please enable it to use appointment reminders.": "Notifications-appen er ikke aktiveret. Aktivér den for at bruge aftalepåmindelser.",
        "You can enable it in the Apps section of your Nextcloud settings.": "Du kan aktivere den i Apps-sektionen i dine Nextcloud-indstillinger.",
        "Select groups…": "Vælg grupper…",
        "%n group selected": "%n gruppe valgt",
        "%n groups selected": "%n grupper valgt",
        "Check-in": "Check-in",
        "Start check-in": "Start check-in",
        "Back": "Tilbage",
        "Loading appointment data…": "Indlæser aftaledata…",
        "Error loading appointment": "Fejl ved indlæsning af aftale",
        "Failed to load appointment data": "Kunne ikke indlæse aftaledata",
        "Responses": "Svar",
        "No Response": "Intet svar",
        "Present": "Til stede",
        "Absent": "Fraværende",
        "Summary": "Oversigt",
        "Total Users": "Antal brugere i alt",
        "Responded": "Svaret",
        "Checked in by admin": "Tjekket ind af administrator",
        "Checked in?": "Tjekket ind?",
        "Bulk Actions": "Massehandlinger",
        "Mark All Present": "Marker alle som tilstede",
        "Mark All Absent": "Marker alle som fraværende",
        "Reset All": "Nulstil alle",
        "Search by name…": "Søg efter navn…",
        "Filter by group": "Filtrer efter gruppe",
        "Add admin comment…": "Tilføj administratorkommentar…",
        "Check-in comment": "Check-in kommentar",
        "Add a comment for this check-in…": "Tilføj en kommentar til dette check-in…",
        "Share Link": "Del link",
        "Link copied to clipboard": "Link kopieret til udklipsholder",
        "Check-in:": "Check-in:",
        "Comment:": "Kommentar:",
        "All": "Alle",
        "All Present": "Alle tilstede",
        "All Absent": "Alle fraværende",
        "No appointments found": "Ingen aftaler fundet",
        "Loading…": "Indlæser…",
        "Comment (optional)": "Kommentar (valgfri)",
        "Comment could not be saved": "Kommentar kunne ikke gemmes",
        "All Comments": "Alle kommentarer",
        "No responses yet": "Ingen svar endnu",
        "Error loading appointments": "Fejl ved indlæsning af aftaler",
        "Error creating appointment": "Fejl ved oprettelse af aftale",
        "Please enter an appointment name": "Indtast venligst et aftalenavn",
        "Please select a start date and time": "Vælg venligst en startdato og -tid",
        "Please select an end date and time": "Vælg venligst en slutdato og -tid",
        "Error updating appointment": "Fejl ved opdatering af aftale",
        "Error deleting appointment": "Fejl ved sletning af aftale",
        "Error updating response": "Fejl ved opdatering af svar",
        "Failed to save response": "Svar kunne ikke gemmes",
        "Appointment created successfully": "Aftale oprettet",
        "Appointment updated successfully": "Aftale opdateret",
        "Appointment deleted successfully": "Aftale slettet",
        "Response updated successfully": "Svar opdateret",
        "Are you sure you want to delete this appointment?": "Er du sikker på, at du vil slette denne aftale?",
        "Past Appointments": "Tidligere aftaler",
        "Created by": "Oprettet af",
        "Created on": "Oprettet den",
        "Updated on": "Opdateret den",
        "No responses from this group yet": "Ingen svar fra denne gruppe endnu",
        "No response yet:": "Intet svar endnu:",
        "Non-responding users": "Brugere uden svar",
        "Show all appointments": "Vis alle aftaler",
        "Add comment": "Tilføj kommentar",
        "Optional: Reason for your response": "Valgfrit: Begrundelse for dit svar",
        "Confirm": "Bekræft",
        "Attendance Settings": "Deltagelsesindstillinger",
        "Group Access Control": "Gruppeadgangskontrol",
        "Groups": "Grupper",
        "Settings saved successfully": "Indstillinger gemt",
        "Failed to load settings": "Kunne ikke indlæse indstillinger",
        "Failed to save settings": "Kunne ikke gemme indstillinger",
        "All attendees checked in": "Alle deltagere tjekket ind",
        "{count} attendees not yet checked in": "{count} deltagere endnu ikke tjekket ind",
        "Confirm Bulk Action": "Bekræft massehandling",
        "Do you really want to set all {count} users to {action}?": "Vil du virkelig sætte alle {count} brugere til {action}?",
        "attending": "tilstede",
        "not attending": "ikke tilstede",
        "Export": "Eksporter",
        "Export created successfully: {filename}": "Eksport oprettet: {filename}",
        "Failed to export appointments": "Kunne ikke eksportere aftaler",
        "Visible to": "Synlig for",
        "Leave empty to show appointment to all users": "Lad stå tomt for at vise aftalen til alle brugere",
        "Search users, groups or teams…": "Søg brugere, grupper eller teams…",
        "Response Summary Groups": "Grupper til svaroversigt",
        "Select which groups to include in response summaries. Users outside these groups will appear under Others. Leave empty to include all groups.": "Vælg hvilke grupper der skal vises i svaroversigten. Brugere uden for disse grupper vises under Andre. Efterlad tomt for at inkludere alle grupper.",
        "Restrict Access": "Begræns adgang",
        "Limits who can see this appointment. Leave empty for all users.": "Begrænser hvem der kan se denne aftale. Lad stå tomt for alle brugere.",
        "Some selections are not in the Response Summary Groups/Teams and may therefore appear under \"Others\".": "Nogle valg er ikke i svaroversigt-grupperne/holdene og kan derfor blive vist under \"Andre\".",
        "Configure in Admin Settings": "Konfigurer i adminindstillinger",
        "Notification": "Besked",
        "Send notification": "Send besked",
        "Notify users who can see this appointment about its creation": "Giv besked til brugere, der kan se denne aftale, om at den er oprettet",
        "New appointment: %s on %s": "Ny aftale: %s den %s",
        "A new appointment has been created. Please respond soon.": "En ny aftale er oprettet. Svar venligst snart.",
        "Response missing: %s on %s": "Svar mangler: %s den %s",
        "Please respond to the upcoming appointment!": "Venligst svar på den kommende aftale!",
        "Group": "Gruppe",
        "Calendar Subscription": "Kalenderabonnement",
        "Subscribe to your appointments in external calendar apps like Google Calendar, Apple Calendar, or Thunderbird.": "Abonner på dine aftaler i eksterne kalender-apps som Google Kalender, Apple Kalender eller Thunderbird.",
        "Subscription URL": "Abonnement-URL",
        "iCal Subscription URL": "iCal-abonnement-URL",
        "Copy URL": "Kopier URL",
        "Copy": "Kopier",
        "Last accessed": "Sidst tilgået",
        "Keep this URL private! Anyone with this link can see your appointments.": "Hold denne URL hemmelig! Alle med dette link kan se dine aftaler.",
        "Regenerate URL": "Generer ny URL",
        "Regenerate Subscription URL?": "Generer ny abonnement-URL?",
        "This will invalidate your current calendar subscription URL. Any calendar apps using the old URL will need to be updated with the new URL.": "Dette vil ugyldiggøre din nuværende kalenderabonnement-URL. Alle kalender-apps, der bruger den gamle URL, skal opdateres med den nye URL.",
        "Regenerate": "Generer ny",
        "URL copied to clipboard": "URL kopieret til udklipsholder",
        "Failed to copy URL": "Kunne ikke kopiere URL",
        "Subscription URL regenerated successfully": "Abonnement-URL blev genereret succesfuldt",
        "Failed to regenerate subscription URL": "Kunne ikke generere ny abonnement-URL",
        "Attendance Appointments": "Deltagelsesaftaler",
        "Your response": "Dit svar",
        "View or change your response": "Se eller ændr dit svar",
        "Not responded": "Ikke besvaret",
        "Please respond! We still need your answer.": "Venligst svar! Vi mangler stadig dit svar.",
        "Me": "Mig",
        "Quick subscribe": "Hurtig abonnering",
        "Google Calendar": "Google Kalender",
        "Apple Calendar": "Apple Kalender",
        "Quick Response": "Hurtig svar",
        "Confirm Response": "Bekræft svar",
        "Thank you for your response!": "Tak for dit svar!",
        "Your response has been saved successfully.": "Dit svar er blevet gemt succesfuldt.",
        "Unable to process response": "Kunne ikke behandle svar",
        "Invalid response type.": "Ugyldig svartype.",
        "This link is invalid. Please use the link from your notification.": "Dette link er ugyldigt. Brug venligst linket fra din notifikation.",
        "This link has expired. The appointment has already ended.": "Dette link er udløbet. Aftalen er allerede afsluttet.",
        "This link is no longer valid.": "Dette link er ikke længere gyldigt.",
        "This appointment no longer exists.": "Denne aftale eksisterer ikke længere.",
        "An error occurred while recording your response. Please try again.": "Der opstod en fejl ved registrering af dit svar. Prøv venligst igen.",
        "Your response:": "Dit svar:",
        "You are about to respond with": "Du er ved at svare med",
        "You are answering as": "Du svarer som",
        "Open Attendance App": "Åbn Deltager-app",
        "View Appointment Details": "Se aftaledetaljer",
        "Confirm your response?": "Bekræft dit svar?",
        "Attachments": "Vedhæftede filer",
        "Add from Files": "Tilføj fra Filer",
        "Choose files to attach": "Vælg filer at vedhæfte",
        "Attach": "Vedhæft",
        "Attachment added": "Vedhæftet fil tilføjet",
        "Attachment removed": "Vedhæftet fil fjernet",
        "Failed to add attachment": "Kunne ikke tilføje vedhæftet fil",
        "Failed to remove attachment": "Kunne ikke fjerne vedhæftet fil",
        "No attachments": "Ingen vedhæftede filer",
        "Files that are important for this appointment can be selected here.": "Filer, der er vigtige for denne aftale, kan vælges her.",
        "Copy Appointment": "Kopiér aftale",
        "Date & Time": "Dato og tid",
        "_%n appointment awaiting your response_::_%n appointments awaiting your response_": ["%n aftale venter på dit svar","%n aftaler venter på dit svar"],
        "Hurray! You responded to all appointments.": "Hurra! Du har svaret på alle aftaler.",
        "Show upcoming appointments": "Vis kommende aftaler",
        "Add your comment…": "Din kommentar…",
        "Bold": "Fed",
        "Italic": "Kursiv",
        "Strikethrough": "Gennemstreget",
        "Bullet list": "Punktliste",
        "Numbered list": "Nummereret liste",
        "Quote": "Citat",
        "Insert link": "Indsæt link",
        "Supports Markdown formatting": "Understøtter Markdown-formatering",
        "Write your description here…": "Skriv din beskrivelse her…",
        "User": "Bruger",
        "Team": "Hold",
        "Response Summary Teams": "Svaroversigt hold",
        "Select which teams to include in response summaries. Team members will be grouped together like regular groups.": "Vælg hvilke hold der skal inkluderes i svaroversigter. Holdmedlemmer vil blive grupperet ligesom almindelige grupper.",
        "Search and select teams…": "Søg og vælg hold…",
        "%n team selected": "%n hold valgt",
        "%n teams selected": "%n hold valgt",
        "Import from Calendar": "Importer fra kalender",
        "Select a calendar event to pre-fill the appointment form.": "Vælg en kalenderbegivenhed for at udfylde aftaleformularen automatisk.",
        "Loading calendars…": "Indlæser kalendere…",
        "Select Calendar": "Vælg kalender",
        "No calendars found": "Ingen kalendere fundet",
        "Loading events…": "Indlæser begivenheder…",
        "Select Event": "Vælg begivenhed",
        "Untitled Event": "Unavngivet begivenhed",
        "No upcoming events in this calendar": "Ingen kommende begivenheder i denne kalender",
        "Failed to load calendars": "Kunne ikke indlæse kalendere",
        "Failed to load calendar events": "Kunne ikke indlæse kalenderbegivenheder",
        "Imported from Calendar": "Importeret fra kalender",
        "Calendar Sync": "Kalendersynkronisering",
        "Automatically update attendance appointments when their linked calendar events are modified.": "Opdater automatisk fremmødetermer, når deres tilknyttede kalenderbegivenheder ændres.",
        "Calendar sync requires Nextcloud 32 or newer.": "Kalendersynkronisering kræver Nextcloud 32 eller nyere.",
        "This feature uses calendar event hooks that are only available in Nextcloud 32 and later versions.": "Denne funktion bruger kalenderbegivenheds-hooks, som kun er tilgængelige i Nextcloud 32 og senere versioner.",
        "Enable automatic calendar sync": "Aktivér automatisk kalendersynkronisering",
        "When enabled, changes to calendar events will automatically update linked attendance appointments (title, description, date/time).": "Når aktiveret, vil ændringer i kalenderbegivenheder automatisk opdatere tilknyttede fremmødeterminer (titel, beskrivelse, dato/tid).",
        "Linked to Calendar": "Tilknyttet kalender",
        "This appointment is linked to a calendar event.": "Denne aftale er tilknyttet en kalenderbegivenhed.",
        "Auto-sync enabled": "Auto-synkronisering aktiveret",
        "Changes to the calendar event will not be synced.": "Ændringer i kalenderbegivenheden vil ikke blive synkroniseret.",
        "Enable auto-sync": "Aktivér auto-synkronisering",
        "Changes to the calendar event will automatically overwrite title, description, and date/time of this appointment.": "Ændringer i kalenderbegivenheden vil automatisk overskrive titel, beskrivelse og dato/tid for denne aftale.",
        "Search calendars…": "Søg kalendere…",
        "No calendars match your search": "Ingen kalendere matcher din søgning",
        "Search events…": "Søg begivenheder…",
        "No events match your search": "Ingen begivenheder matcher din søgning"
    },
    "nplurals=2; plural=(n != 1);"
);
